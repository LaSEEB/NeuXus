...
if __name__ == '__main__':
    ...
    chans = select.ChannelSelector(reref.output, 'index', [1, 2, 3])
    ...
    log_epoch = function.ApplyFunction(
        time_epoch.output, lambda x: np.log1p(x))

    baseline = epoching.StimulationBasedEpoching(
        average_epoch.output, lsl_marker_reception.output, 800, 0.025, 3)
    ...
    relative_band = function.ApplyFunction(
        average_epoch.output, relative_alpha)
